...
% generate zero-median sequence
g = x;
x_median = median(g);
x = bsxfun(@minus,g,x_median);
r = r*std(x);
% compute the embedded vector with dimension m only
N = length(x);
xMat = zeros(m,N-m+1);
% xMat = zeros(m+1,N-m);
for i = 1:m
...
n_max = length(xMat);
B_local = zeros(1,n_max - 1);
for i = 1:n_max - 1
    clearvars Ang_dis
    %Compute angular distance for all pairwise embedding vectors
    tempdis= bsxfun(@angdis,xMat(:,i),xMat(:,(i+1):n_max));
    Ang_dis = tempdis(1,:);
    B_local(i) = length(Ang_dis(Ang_dis<=r))/length(Ang_dis);
%     disp(num2str(i));
end
B_global = sum(B_local)/length(B_local);
CSE = -(B_global*log2(B_global)+(1-B_global)*log2(1-B_global));

end


function out = angdis(x,y)
cossim = x.'*y/(norm(x)*norm(y));
angdistance = acos(cossim)/pi;
out = ones(length(x),1)*angdistance;
end