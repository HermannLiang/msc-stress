function [hrv,stage] = gethrv(subjectno,visual,filtermethod)
...
%R peaks detection
%%%%%%%%%%%%%Another Implementation Using the Pam-tompkins algorithm
[qrs_amp_raw,qrs_i_raw,delay]=pan_tompkin(ecg,fs,0);


%%%%%%%%%%%%%%ONLY ONE FILTERING%%%%%%%%%%%%%%
% Filter - first pass
WinSize = floor(fs * 600 / 1000);% You may change 571 to a proper val
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            plot(timeaxis(filtered1 ~= 0),filtered1(filtered1 ~= 0),'x');
            xlim([tst, ted]);
            xlabel('seconds');
...
index = (noisyhrv > 1.2)|(noisyhrv<0.3);
...
win_excluded = [noisyhrv(find(index)-4);noisyhrv(find(index)-3);noisyhrv(find(index)-2);...
    ...
    noisyhrv(find(index)+3);noisyhrv(find(index)+4)];
        ...
        width=550;
        height=400;
        set(gcf,'units','pixel','position',[x0,y0,width,height])
%             subplot(3,2,5)
            plot(noisyhrv);
            hold on
            plot(find(index),noisyhrv(index),'+'); 
            xlabel('seconds');
            title(['Noisy HRV, subject: ', num2str(subjectno)])
            end
...
legend('hrv','outliers','global filter','local filter','Location','northeast')
end
hrv = noisyhrv;
end

%filter method 2 via median filter
if filtermethod ==2
...
hrv_time = qrs_i_raw(2:end)./fs; 
hrvaxis_interp = 1/rr_fs:1/rr_fs:length(ecg)/fs;
