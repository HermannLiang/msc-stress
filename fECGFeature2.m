function feats = fECGFeature2(data,fs)
% Note:
% Input:
...
for loop = 1:size(data,1)
    ...
    % Removing lower frequencies
                ...
                %     fresult(end - round(length(fresult)*5/fs) : end)=0;
                    ...
                    figure(1)
                    subplot(3,2,1)
                    plot(ecg_signal);xlim([0,floor(size(data,2)./50)]); 
                    title('unfiltered')
                    subplot(3,2,2)
                    plot(corrected);xlim([0,floor(size(data,2)./50)]);
                    title('filtered')
                % 
                    figure,
            ...
            %Visualization
            subplot(3,2,3)
            plot(filtered1);xlim([0,floor(size(data,2)./50)]);
...
%     % Visualization
    subplot(3,2,4)
    plot(peaks1);xlim([0,floor(size(data,2)./50)]);
    ...
    subplot(3,2,5)
    plot(peaks2);xlim([0,floor(size(data,2)./50)]);
    % Visualization
%     figure(2)
...
%     title('\bf6. Detected Peaks - Finally');  xlim([0,floor(size(data,2)./10)]);
    ...
    hrv_time_interp = 1/rr_fs:1/rr_fs:length(ecg_signal)/fs;%60 time axis in seconds after interpolation
